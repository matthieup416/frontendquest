...
function MessagesScreen(props) {
  ...
  useEffect(() => {
    async function selectedConversation() {
      const data = await fetch(
        `http://${MY_IP}:3000/inbox/selectedQuest?id=${selectedQuest}&token=${props.dataUser.token}`
      )
      const body = await data.json()
  ...
  var listMsgConversation = async (id) => {
    ...
    const data = await fetch(
      `http://${MY_IP}:3000/inbox/conversation?id=${id}&token=${props.dataUser.token}`
    )
    const body = await data.json()
    ...
    if (msgIsVisible) {
      return (
        <View style={styles.container}>
          ...
          >
            <Image
              source={{ uri: offerMessages[0].pictures[0].url }}
              style={{ width: 100, height: 60, marginHorizontal: 20 }}
              resizeMethod="resize"
              resizeMode="center"
            ></Image>
            <View
              style={{ flex: 1, flexDirection: "column", color: "#FFFFFF" }}
            >
              <Text
                style={{ color: "#FFFFFF", fontWeight: "700", fontSize: 16 }}
              >
                {offerMessages[0].type +
                  " " +
                  offerMessages[0].nb_pieces +
                  " pièces " +
                  offerMessages[0].surface +
                  "m²"}
              </Text>
              <Text
                style={{ color: "#FFFFFF", fontWeight: "700", fontSize: 16 }}
              >
                {offerMessages[0].price
                  .toString()
                  .replace(/\B(?=(\d{3})+(?!\d))/g, " ")}{" "}
                €
              </Text>
              ...
              >
                {offerMessages[0].city}
              </Text>
            </View>
            <TouchableOpacity
              onPress={() => listMsgConversation(selectedConversation)}
              style={{ marginRight: 10, flex: 0, justifyContent: "center" }}
            >
          ...
          <KeyboardAvoidingView>
            <TextInput
              ...
              value={inputMessage}
              style={{ padding: 10, backgroundColor: "#F8F7FF" }}
